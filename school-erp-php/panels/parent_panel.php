 ...
 * Parent Panel
 */
// Role guard — only parent (admin/superadmin bypass via role_matches)
if (!role_matches(get_current_role(), ['parent'])) {
    header('Location: ' . BASE_URL . '/dashboard.php'); exit;
}
$me = get_authenticated_user();

// First-login banner flag (shown when account was auto-generated and never changed password)
$_parentFirstLogin = false;
if (!empty($me)) {
    $puRow = db_fetch("SELECT portal_generated, last_login_at FROM users WHERE id=?", [$me['id']]);
    $_parentFirstLogin = !empty($puRow['portal_generated']) && empty($puRow['last_login_at']);
    // Mark as logged in (suppress banner after first visit)
    if ($_parentFirstLogin) {
        db_query("UPDATE users SET last_login_at=NOW() WHERE id=?", [$me['id']]);
    }
}
...
<?php if ($_parentFirstLogin): ?>
<!-- First Login Banner — auto-dismisses after first view -->
<div style="background:linear-gradient(135deg,rgba(251,191,36,.15),rgba(251,163,36,.08));border:1px solid rgba(251,191,36,.4);border-radius:12px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:center;gap:14px">
    <div style="font-size:28px">🔐</div>
    <div style="flex:1">
        <div style="font-weight:700;font-size:14px;color:#fbbf24">Welcome! Please change your password</div>
        <div style="font-size:12px;color:#94a3b8;margin-top:3px">Your account was auto-generated by the school. For security, please update your password from your profile settings.</div>
    </div>
    <a href="<?= BASE_URL ?>/profile.php" style="background:#fbbf24;color:#000;padding:8px 16px;border-radius:8px;font-size:12px;font-weight:700;text-decoration:none;white-space:nowrap">Change Password →</a>
</div>
<?php endif; ?>
