...
if (!defined('APP_URL'))
    define('APP_URL', 'http://localhost/school-erp-php');
...
<body>
    <div class="container">
        ...
        <div class="page-sub">Environment: <strong><?= htmlspecialchars(APP_ENV) ?></strong> &nbsp;|&nbsp; Server:
            <strong><?= htmlspecialchars($_SERVER['SERVER_NAME'] ?? 'localhost') ?></strong> &nbsp;|&nbsp; Time:
            <strong><?= date('d M Y H:i:s') ?></strong>
        </div>

        <div class="delete-warning">
            ⚠️ <strong>IMPORTANT:</strong> Delete or password-protect this file before deploying to production! URL:
            <code>diagnostic.php?access=erp2025</code>
        </div>
        ...
        <div class="status-banner <?= $allGood ? 'ok' : 'fail' ?>">
            ...
            <div>
                <div class="status-text <?= $allGood ? 'ok' : 'fail' ?>">
                    <?= $allGood ? 'System Healthy — All checks passed!' : 'Issues Detected — See details below' ?>
                </div>
                <div class="status-sub">
                    ...
                    if (!empty($missingTables))
                        $issues[] = count($missingTables) . ' missing table(s)';
                    if (!empty($missingFiles))
                        $issues[] = count($missingFiles) . ' missing file(s)';
                    if (!empty($missingExts))
                        $issues[] = count($missingExts) . ' missing PHP extension(s)';
        ...
        <div class="grid">
            ...
            <div class="card">
                <div class="card-head">
                    ...
                    <span
                        class="badge <?= $dbConnected ? 'badge-ok' : 'badge-fail' ?>"><?= $dbConnected ? 'CONNECTED' : 'FAILED' ?></span>
                </div>
                <div class="card-body">
                    <table>
                        <tr>
                            <td>Host</td>
                            <td class="mono"><?= htmlspecialchars(DB_HOST) ?></td>
                        </tr>
                        <tr>
                            <td>Database</td>
                            <td class="mono"><?= htmlspecialchars(DB_NAME) ?></td>
                        </tr>
                        <tr>
                            <td>User</td>
                            <td class="mono"><?= htmlspecialchars(DB_USER) ?></td>
                        </tr>
                        <tr>
                            <td>Charset</td>
                            <td class="mono"><?= htmlspecialchars(DB_CHARSET) ?></td>
                        </tr>
                        <tr>
                            <td>Status</td>
                            <td>
                                ...
                                <?php else: ?>
                                    ...
                                    <div style="font-size:11px;color:#dc2626;margin-top:4px;font-family:monospace">
                                        <?= htmlspecialchars($dbError ?? '') ?>
                                    </div>
                                    <div style="font-size:11px;color:#666;margin-top:6px">
                                        Fix: Create database "<strong><?= htmlspecialchars(DB_NAME) ?></strong>" in
                                        phpMyAdmin,<br>
                        ...
                        <?php if ($dbConnected): ?>
                            <tr>
                                <td>Total Tables</td>
                                <td class="mono"><?= count($existingTables) ?> found</td>
                            </tr>
                        <?php endif; ?>
                    </table>
                </div>
            ...
            <div class="card">
                <div class="card-head">
                    ...
                    <span
                        class="count-chip <?= empty($missingExts) ? 'green' : 'red' ?>"><?= count($extStatus) - count($missingExts) ?>/<?= count($extStatus) ?>
                        loaded</span>
                </div>
                <div class="card-body">
                    <table>
                        <?php foreach ($extStatus as $ext => $loaded): ?>
                            <tr>
                                ...
                                <td><span
                                        class="badge <?= $loaded ? 'badge-ok' : 'badge-fail' ?>"><?= $loaded ? 'OK' : 'MISSING' ?></span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </table>
                    ...
                    <table>
                        <?php foreach ($phpSettings as $key => $info): ?>
                            <tr>
                                ...
                                <td class="mono"><span
                                        class="badge <?= $info['ok'] ? 'badge-ok' : 'badge-warn' ?>"><?= htmlspecialchars($info['value']) ?></span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </table>
                ...
                <div class="card-body">
                    <table>
                        <tr>
                            ...
                            <td>
                                <span
                                    class="badge <?= $uploadsWritable ? 'badge-ok' : 'badge-fail' ?>"><?= $uploadsWritable ? 'WRITABLE' : 'NOT WRITABLE' ?></span>
                                <?php if (!$uploadsWritable): ?>
                                    <div style="font-size:11px;color:#dc2626;margin-top:4px">Create or chmod 755 the
                                        uploads/ folder</div>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            <td>tmp/ directory</td>
                            <td><span
                                    class="badge <?= $tmpWritable ? 'badge-ok' : 'badge-warn' ?>"><?= $tmpWritable ? 'WRITABLE' : 'CHECK' ?></span>
                            </td>
                        </tr>
                        <tr>
                            <td>.env.php</td>
                            <td><span
                                    class="badge <?= file_exists(__DIR__ . '/.env.php') ? 'badge-ok' : 'badge-fail' ?>"><?= file_exists(__DIR__ . '/.env.php') ? 'EXISTS' : 'MISSING' ?></span>
                            </td>
                        </tr>
                        <tr>
                            <td>APP_ENV</td>
                            <td><span
                                    class="badge <?= APP_ENV === 'local' ? 'badge-info' : 'badge-warn' ?>"><?= htmlspecialchars(APP_ENV) ?></span>
                            </td>
                        </tr>
                        <tr>
                            <td>APP_DEBUG</td>
                            <td><span
                                    class="badge <?= filter_var(APP_DEBUG, FILTER_VALIDATE_BOOLEAN) ? 'badge-warn' : 'badge-ok' ?>"><?= htmlspecialchars(APP_DEBUG) ?></span>
                            </td>
                        </tr>
                    </table>
                </div>
        ...
        <!-- Database Tables -->
        <div class="section-label">Database Tables
            (<?= count($tableStatus) - count($missingTables) ?>/<?= count($tableStatus) ?> exist)</div>
                    ...
                    <tbody>
                        <?php foreach ($tableStatus as $table => $info): ?>
                            <tr>
                                ...
                                <td><span
                                        class="badge <?= $info['exists'] ? 'badge-ok' : 'badge-fail' ?>"><?= $info['exists'] ? 'OK' : 'MISSING' ?></span>
                                </td>
        ...
        <!-- Module Files -->
        <div class="section-label">Module Files
            (<?= count($criticalModules) - count(array_filter($fileStatus, fn($v, $k) => !$v && array_key_exists($k, $criticalModules), ARRAY_FILTER_USE_BOTH)) ?>/<?= count($criticalModules) ?>
            present)</div>
        <div class="grid">
            <div class="card">
                <div class="card-head">
                        ...
                        <tbody>
                            <?php foreach ($criticalModules as $file => $desc): ?>
                                <tr>
                                    ...
                                    <td><span
                                            class="badge <?= $fileStatus[$file] ? 'badge-ok' : 'badge-fail' ?>"><?= $fileStatus[$file] ? 'OK' : 'MISSING' ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
        ...
        <?php if (!$dbConnected || !empty($missingTables)): ?>
            ...
            <div class="card full-card">
                <div class="card-head">
                    <div class="card-title">🛠 How to Fix Database Issues</div>
                </div>
                <div class="card-body" style="padding:18px">
                    <ol style="padding-left:20px;line-height:2;font-size:14px">
                        <li>Open <strong>XAMPP Control Panel</strong> → Start <strong>Apache</strong> and
                            <strong>MySQL</strong>
                        </li>
                        <li>Open <a href="http://localhost/phpmyadmin" target="_blank"
                                style="color:#2563eb">http://localhost/phpmyadmin</a></li>
                        <li>Click <strong>New</strong> on the left sidebar → Create database:
                            <code><?= htmlspecialchars(DB_NAME) ?></code> with charset <code>utf8mb4_unicode_ci</code>
                        </li>
                        <li>Select the database <code><?= htmlspecialchars(DB_NAME) ?></code> → Click
                    ...
                    <p style="margin-top:12px;font-size:13px;color:#666">
                        After import, login at: <a href="<?= htmlspecialchars(APP_URL) ?>" target="_blank"
                            style="color:#2563eb"><?= htmlspecialchars(APP_URL) ?></a><br>
                        Username: <code>admin@school.com</code> &nbsp; Password: <code>Password@123</code> (change
                        immediately!)
                    </p>
                </div>
            </div>
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:32px">
            <a href="<?= htmlspecialchars(APP_URL) ?>/" target="_blank"
                style="text-decoration:none;background:#0a0a0a;color:#fff;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:500">🏠
                Open App</a>
            <a href="http://localhost/phpmyadmin" target="_blank"
                style="text-decoration:none;background:#f59e0b;color:#fff;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:500">🗄️
                phpMyAdmin</a>
            <a href="<?= htmlspecialchars(APP_URL) ?>/api/health.php" target="_blank"
                style="text-decoration:none;background:#2563eb;color:#fff;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:500">📡
                API Health JSON</a>
            <a href="<?= htmlspecialchars(APP_URL) ?>/diagnostic.php?access=erp2025"
                style="text-decoration:none;background:#6b7280;color:#fff;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:500">🔄
                Refresh</a>
        </div>

        <div style="font-size:12px;color:#aaa;text-align:center">
            School ERP v<?= APP_VERSION ?> &middot; Diagnostic Page &middot; <?= date('Y') ?> &middot; <strong>DELETE
                BEFORE PRODUCTION</strong>
        </div>
