...
// Require CLI and admin authentication
if (php_sapi_name() !== 'cli') {
    http_response_code(403);
    die('This script can only be run from the command line.');
}

// Verify user is authenticated if running via web (shouldn't happen, but defense in depth)
if (!is_cli_request()) {
    require_auth();
    require_role(['superadmin', 'admin']);
}

// Helper: check if running from CLI
function is_cli_request()
{
    return php_sapi_name() === 'cli';
}

// Helper: create temporary my.cnf file for mysqldump (avoids password in process list)
function createTempMyCnf()
{
    $tempFile = tempnam(sys_get_temp_dir(), 'mycnf_');
    $content = "[client]\nhost=" . DB_HOST . "\nuser=" . DB_USER . "\npassword=" . DB_PASS . "\n";
...
if ($action === 'backup' || $action === '--backup') {
    ...
    // Build mysqldump command (use env file for password, not CLI args)
    $command = sprintf(
        'mysqldump --defaults-file=%s %s --single-transaction --quick --lock-tables=false > %s 2>&1',
        escapeshellarg(self::createTempMyCnf()),
        escapeshellarg(DB_NAME),
        escapeshellarg($filepath)
    );

    // Try mysqldump first
    $myCnf = createTempMyCnf();
    $command = sprintf(
        'mysqldump --defaults-file=%s %s --single-transaction --quick --lock-tables=false > %s 2>&1',
    ...
    @unlink($myCnf); // Clean up temp config immediately
