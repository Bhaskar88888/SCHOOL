...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $data = get_post_json();
    $id = db_insert(
        "INSERT INTO routine (class_id, day, subject, teacher_id, start_time, end_time, room) VALUES (?,?,?,?,?,?,?)",
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    $id = (int) $data['id'];
    db_query(
        "UPDATE routine SET class_id=?, day=?, subject=?, teacher_id=?, start_time=?, end_time=?, room=? WHERE id=?",
        [(int) $data['class_id'], sanitize($data['day']), sanitize($data['subject']), (int) ($data['teacher_id'] ?? 0), $data['start_time'] ?? null, $data['end_time'] ?? null, sanitize($data['room'] ?? ''), $id]
    );
