...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $allocations = [];
    try { $allocations = db_fetchAll("SELECT * FROM transport_allocations"); } catch(Exception $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    if ($type === 'vehicle') {
        $id = (int)$data['id'];
        db_query("UPDATE transport_vehicles SET vehicle_no=?, type=?, capacity=?, driver_name=?, driver_phone=? WHERE id=?",
        ...
        $id = (int)$data['id'];
        db_query("UPDATE bus_routes SET route_name=?, vehicle_id=?, stops=?, monthly_fee=? WHERE id=?",
