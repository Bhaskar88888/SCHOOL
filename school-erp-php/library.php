...
async function loadBooks() {
    ...
    document.getElementById('booksBody').innerHTML = bookList.map(b => `
        <tr>
            ...
            <td>${b.available_copies>0 ? `<button class="btn btn-primary btn-sm" onclick="openIssue(${b.id})">Issue</button>` : ''}
                <button class="btn btn-danger btn-sm" onclick="deleteBook(${b.id})">🗑️</button></td>
        </tr>
...
async function submitIssue(e) {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(document.getElementById('issueBookForm')));
    const res = await apiPost('/api/library/index.php', data);
    if(res.success){ showToast('Book Issued successfully'); closeModal('issueBookModal'); document.getElementById('issueBookForm').reset(); switchTab('issues'); }
    else showToast(res.error||'Error','danger');
}
